...
public class MainActivity extends AppCompatActivity {
    ...
    public void login() {
        ...
        auth.signInAnonymously().addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            // auth.signInWithEmailAndPassword("578939729@qq.com","12345678").addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            @Override
            public void onComplete(Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    //身份认证成功
                    String uid = auth.getCurrentUser().getUid();
                    //用户可以使用任意自定义节点来保存用户数据
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE && resultCode == PermissionHelper.PERMISSIONS_DENIED) {
            Toast.makeText(MainActivity.this, "用户拒绝授权,缺少主要权限,无法运行,退出应用", Toast.LENGTH_SHORT).show();
            finish();
        } else {
